...
import { TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Paper, makeStyles } from '@material-ui/core'
...
const ItemCategoryManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    const getMcndData = async () => {
      let response = await fetch('getMcndDivData', { method: 'GET' })
      if (response.status !== 200) {
        alert('商品区分情報の取得に失敗しました。')
      }
      let resjson = await response.json()
      let rowsAll: rowTyped[] = []
      resjson.res.map((row: rowTyped) => {
        rowsAll.push({
          MCND_DIV_ID: row.MCND_DIV_ID,
  ...
  const addRow = () => {
    ...
    rowsAll.push({
      MCND_DIV_ID: nowRow.slice(-1)[0].MCND_DIV_ID + 1,
      MCND_DIV_NAME: '',
      newFlg: true,
    })
    ...
    if (selectedRows.length !== 0) {
      for (let i = 0; i < selectedRows.length; i++) {
        ...
        if (firstMaxIndex >= Number(selectedRows[i].id)) {
          ...
          if (i === 0) {
            // 初回は、現在行から選択行以外を切り出し
            rowsAll = nowRow.filter((row) => Number(selectedRows[i].id) !== row.MCND_DIV_ID)
          } else {
            // 2回目以降は、切り出し済みの行から選択行以外を切り出し
            rowsAll = rowsAll.filter((row) => Number(selectedRows[i].id) !== row.MCND_DIV_ID)
          }
        }
      }
      // 0行になる場合、削除しない
  ...
  const registerData = async () => {
    ...
    if (selectedRows.length !== 0) {
      for (let i = 0; i < selectedRows.length; i++) {
        ...
        rowsAll.push(
          nowRow.filter(
            (row) => Number(selectedRows[i].id) === row.MCND_DIV_ID && row.MCND_DIV_NAME && row.MCND_DIV_NAME
          )[0]
        )
      }
      const reqestOptions = {
      ...
      // 登録処理実行
      let response = await fetch('itemDivResister', reqestOptions)
      if (response.status !== 200) {
        alert('商品区分の登録に失敗しました。')
      } else {
        alert('登録完了しました。')
      }
    } else {
  ...
  const deleteData = async () => {
    ...
    if (selectedRows.length !== 0) {
      for (let i = 0; i < selectedRows.length; i++) {
        ...
        rowsAll.push(nowRow.filter((row) => Number(selectedRows[i].id) === row.MCND_DIV_ID && row.newFlg === false)[0])
      }
      const reqestOptions = {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // 削除処理実行
      let response = await fetch('itemDivDelete', reqestOptions)
      if (response.status !== 200) {
        alert('商品区分の削除に失敗しました。')
      } else {
        alert('削除完了しました。')
      }
    } else {
  ...
  return (
    <GenericTemplate title="商品区分管理画面">
      <form>
              ...
              {nowRow !== null &&
                nowRow.map((row) => (
                  <TableRow
                    hover
                    role="checkbox"
                    id={row.MCND_DIV_ID.toString()}
                    key={row.MCND_DIV_ID}
                    selected={isSelected(row.MCND_DIV_ID)}
                  >
                    <TableCell padding="checkbox">
                      <Checkbox
                        color="primary"
                        checked={isSelected(row.MCND_DIV_ID)}
                        onClick={(event) => handleClick(event, row.MCND_DIV_ID)}
                        inputProps={{
